...
void update_pong(struct Pong* pong, double dt)
{
    if (pong->state == PLAY)
    {
        ...
        if((pong->ball.x < TABLE_WIDTH/2 || pong->ball.vx < 0) || (pong->ball.y > pong->player2.y && pong->ball.y < (pong->player2.y + pong->player2.height)))
        {
            pong->player2.vy = 0;
        }
        ...
        else if((pong->player2.y + pong->player2.height) < pong->ball.y)
        {
            pong->player2.vy = PADDLE_SPEED;
        }
